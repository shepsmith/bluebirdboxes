...
<?php  
	...
	$red = $green = $brown = $blue = $black = $white = $yellow = $pink = $gold = 0;
	...
	if (isset($_POST['submit'])) {
		...
		if(array_filter($errors)) {
			...
			if(isset($_POST['pink_check'])) {
				$test_color = mysqli_real_escape_string($conn, $_POST['pink_check']);
				if(check_color_set($test_color)) {
					$colors[$i] = "pink";
					$i++;
				}
			...
			$x = rand(1,999);
			$sql = "INSERT INTO birdhouse (email, seq, message, design, request_date) 
			...
			foreach ( $colors as $color ) {
				$sql = "INSERT INTO birdhouse_colors (email, seq, color, request_date)
