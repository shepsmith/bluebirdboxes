...
<?php  
	...
	$email = $message = $size = $design = '';
	$red = $green = $brown = $blue = $black = $white = $yellow = $purple = $gold = 0;
	...
	if (isset($_POST['submit'])) {
		...
		if(array_filter($errors)) {
			...
			if(isset($_POST['purple_check'])) {
				$test_color = mysqli_real_escape_string($conn, $_POST['purple_check']);
				if(check_color_set($test_color)) {
					$colors[$i] = "purple";
					$i++;
				}
			...
			if(isset($_POST['design'])) {
				...
				switch($design_num){
					...
					break;
					case 3: 
						$design = "cathedral window";
					break;
					case 4: 
						$design = "white henloon";
					break;
					case 2: 
						$design = "mariners compass";
					break;
					case 6: 
						$design = "log cabin dalia";
					break;
					case 7: 
						$design = "log cabin";
					break;
					case 8: 
						$design = "nine patch star";
					break;
					case 9: 
						$design = "bright ocean wave";
					break;
					case 10: 
						$design = "lone star";
					break;
			...
			$x = rand(1,999);
			$sql = "INSERT INTO quilt (email, seq, message, size, design, request_date) 
			VALUES ('$email', '$x', '$message', '$size', '$design', '$todaysdate' )";
			if(!(mysqli_query($conn, $sql))) {
			...
			foreach ( $colors as $color ) {
				$sql = "INSERT INTO quilt_colors (email, seq, color, request_date)
